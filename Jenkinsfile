pipeline {
  ...
  environment {
    ...
    DEV_URL = "http://10.11.23.191:30080"
  }
  stages {
    stage('Build') {
      parallel {
    ...
    stage('Package') {
      parallel {
        ...
        stage("OCI Image BnP") {
          steps {
            container('kaniko') {
              sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --insecure --skip-tls-verify --cache=true --destination=docker.io/jphalliwell/dso-demo'
            }
          }
        }
      }
        ...
        stage('Image Linting') {
          steps {
            container('docker-tools') {
              sh 'dockle docker.io/jphalliwell/dso-demo'
            }
          }
        }
        stage('Image Scan') {
          steps {
            container('docker-tools') {
              sh 'trivy image --timeout 10m --exit-code 1 jphalliwell/dso-demo'
            }
          }
        }
      }
